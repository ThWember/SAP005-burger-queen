...
export const GetProducts = (state1, state2, state3) => {
  ...
  const idUser =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InRlc3RhbmRvQGhvdG1haWwuY29tIiwiaWQiOjExLCJpYXQiOjE2MTQ5MTU0MDAsImV4cCI6MTY0NjQ3MzAwMH0.LGhyK18HoyJd1TKFNz5ulxENctrc8OegcCEPOCKjMv4";
...
export const Done = (state) => {
  ...
  RequestApi(path, methodType, idUser)
    ...
    .then((result) => {
      console.log(result);
      const drinks = result.filter((itens) => itens.status.includes("Pronto"));
      state(drinks);
    })
    .catch((error) => {
      console.log("error", error);
...
export const handleItem = (
  ...
  if (client === "" || table === "") {
    alert(
      "Primeiro pergunte o nome do cliente e preencha os campos de mesa e nome :)"
    );
  }
...
export const addComplement = (event, accumulator, setAccumulator, clickedItem, setClickedItens, state) => {
  event.preventDefault();
  ...
  chosenProducts[index].qtd_complement = accumulator
  console.log("Complemento", chosenProducts[index].qtd_complement)
  chosenProducts[index].totalPrice += 1;
  state(setClickedItens)
};

export const deleteComplement = (event, accumulator, setAccumulator, clickedItem, setClickedItens, state) => {
  event.preventDefault();
  setAccumulator(accumulator -= 1);
